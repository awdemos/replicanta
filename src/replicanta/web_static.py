...
header{grid-area:head;display:flex;align-items:center;gap:16px;padding:0 16px;border-bottom:1px solid var(--line);background:var(--panel)}
header b{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;letter-spacing:1px}
header b i{color:var(--accent);font-style:normal;font-size:12px}
header b em{font:11px var(--mono);color:var(--muted);letter-spacing:.5px}
header b em [data-name]{color:var(--accent-2)}
...
#local{margin-left:auto;color:var(--accent);font-size:11px}
...
#orgs button.on b{color:#06130a}
#orgs button.on span{color:#06130ab3}
...
.field{position:relative;width:64px;height:64px;flex:0 0 auto;display:grid;place-items:center;background-color:#0a0e15;background-image:linear-gradient(#7ee7870d 1px,transparent 1px),linear-gradient(90deg,#7ee7870d 1px,transparent 1px);background-size:16px 16px;overflow:hidden;border-radius:6px}
.rings{width:60px;height:60px;border-radius:50%;border:1px solid #7ee78726;position:absolute}
.orb{width:52px;height:52px;border-radius:45% 55%;background:radial-gradient(circle at 35% 30%,#e2ffc0,#57a85a 30%,#123620 75%);box-shadow:0 0 24px #7ee78759;display:grid;place-items:center;color:#06130a;font:10px var(--mono);animation:pulse 6s infinite alternate}
...
.meters b{font:13px var(--mono);color:var(--ink);font-variant-numeric:tabular-nums}
...
.msg.org{border-left-color:var(--accent-2);background:#0e1a1c}
.msg.sys{border-left-color:var(--amber);background:#1c1608}
...
#chat button{width:38px;height:38px;border:0;background:var(--accent);color:#06130a;border-radius:6px}
...
.gauge b{font:16px var(--mono);font-variant-numeric:tabular-nums}
.metrics{width:100%;border-collapse:collapse;margin:0 0 14px;font-size:12px}
.metrics th{padding:5px 8px;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;color:var(--muted);text-align:left;border-top:1px solid var(--line);border-bottom:1px solid var(--line)}
.metrics td{padding:6px 8px;border-bottom:1px solid var(--line);color:var(--ink)}
.metrics td.num,.metrics th.num{font-family:var(--mono);font-variant-numeric:tabular-nums;text-align:right;white-space:nowrap}
...
#persona-list button.on{background:var(--accent);color:#06130a}
...
#commandbar button{border:0;background:var(--accent);color:#06130a;width:36px;height:36px;border-radius:6px}
#hints{position:absolute;left:0;right:44px;bottom:calc(100% + 6px);background:var(--panel);border:1px solid var(--line);border-radius:6px;box-shadow:0 4px 12px #000a;display:none;max-height:160px;overflow:auto}
...
#toast{position:fixed;top:16px;right:16px;background:var(--danger);color:#210b09;padding:10px 16px;border-radius:6px;box-shadow:0 4px 12px #000a;opacity:0;pointer-events:none;transition:opacity .3s;z-index:100}
#toast.on{opacity:1}
...
.mud-actor.you{border-left-color:var(--accent-2);background:#0e1a1c}
...
#mud-form button{border:0;background:var(--accent);color:#06130a;width:36px;height:36px;border-radius:6px}
...
function render(s){state=s;$$('[data-name]').forEach(x=>x.textContent=s.organism.name);$('#state').textContent=s.organism.state.toUpperCase();$('#mood').textContent=s.organism.mood;$('#stress').textContent=s.organism.stress;$('#ground').textContent=s.organism.coherence;$('#chaos').value=s.organism.chaos;$('#auto').checked=s.organism.auto_apply;$('#activity').textContent=(s.activity&&s.activity.length)?s.activity[0]:'quiet';
...
else{pp.style.display='none'}$('#voice-toggle').textContent=s.speech.enabled?'voice on':'voice off';const vs=$('#voice-select');vs.innerHTML='<option value="">voice…</option>'+s.speech.voices.map(v=>`<option value="${esc(v)}" ${v===s.speech.current?'selected':''}>${esc(v)}</option>`).join('');vs.disabled=!s.speech.voices.length;$('#self-talk').textContent=s.self_talk?'self-talk on':'self-talk off';$('#git').textContent=s.git_enabled?'git on':'git off';$('#messages').innerHTML=s.chat.map((m,i)=>`<div class="msg ${m.role==='org'?'org':m.role==='system'?'sys':''}"><small>${m.role==='org'?s.organism.name:m.role==='system'?'SYSTEM':'YOU'}<button class="copy" data-idx="${i}">copy</button></small><div>${esc(m.text)}</div>${m.role==='org'?'<button class="why">⌁ How did this form?</button>':''}</div>`).join('');$$('#messages .copy').forEach(b=>b.onclick=()=>copyText(s.chat[b.dataset.idx].text));$('#messages').scrollTop=1e9;$('#beliefs').innerHTML='<h2>What this mind holds to be true</h2>'+(s.beliefs.length?s.beliefs.map(b=>`<article><div><h3>${esc(b.subject)} <em>${esc(b.relation)}</em> ${esc(b.value)}</h3><p>Inspectable symbolic belief</p></div><b>${Math.round(b.confidence*100)}%</b></article>`).join(''):'<p class="empty">No beliefs yet. Chat with the organism to seed its mind.</p>');$('#episodes').innerHTML='<h2>Developmental timeline</h2>'+(s.memory.length?s.memory.map(e=>`<article><div><h3>${esc(e.kind)}</h3><p>${esc(e.text)}</p></div><b>cycle ${e.cycle}</b></article>`).join(''):'<p class="empty">No memories yet.</p>');const ip=$('#inner-panel');const mm=s.mind_metrics||{};const mmRows=[['beliefs',mm.beliefs],['rules',mm.rules],['memories',mm.memories],['cycle',mm.cycle],['consciousness',mm.score],['goals active',mm.goals_active],['goals done',mm.goals_done]];const mmTable='<h3>Mind metrics</h3><table class="metrics"><thead><tr><th>Metric</th><th class="num">Value</th></tr></thead><tbody>'+mmRows.map(r=>'<tr><td>'+esc(r[0])+'</td><td class="num">'+esc(r[1])+'</td></tr>').join('')+'</tbody></table>';const at=s.activity_table||{};const atKeys=Object.keys(at);const atTable=atKeys.length?'<h3>Activity</h3><table class="metrics"><thead><tr><th>Counter</th><th class="num">Total</th><th class="num">/cycle</th></tr></thead><tbody>'+atKeys.map(k=>'<tr><td>'+esc(k)+'</td><td class="num">'+esc(at[k].total)+'</td><td class="num">'+esc(at[k].per_cycle.toFixed(2))+'</td></tr>').join('')+'</tbody></table>':'<p class="empty">No activity yet.</p>';ip.innerHTML='<h2>Inner life</h2><div class="gauges"><div class="gauge"><label>Arousal</label><b>'+s.organism.arousal+'</b></div><div class="gauge"><label>Coherence</label><b>'+s.organism.coherence+'</b></div><div class="gauge"><label>Incoherence</label><b>'+s.organism.incoherence+'</b></div><div class="gauge"><label>Insane</label><b>'+(s.organism.insane?'yes':'no')+'</b></div></div>'+mmTable+atTable;const cp=$('#cells-panel');cp.innerHTML='<h2>Attention & skills</h2><h3>Attention</h3>'+(s.attention.length?s.attention.map(a=>`<span class="chip">${esc(a[0])} → ${esc(a[1])}</span>`).join(''):'<p class="empty">Attention is floating free.</p>')+'<h3>Goals</h3>'+(s.goals.length?s.goals.map(g=>`<article><div><h3>${esc(g.text)}</h3></div><b>${g.done_cycle?'done '+g.done_cycle:'active'}</b></article>`).join(''):'<p class="empty">No goals yet.</p>')+'<h3>Skills</h3>'+(s.skills.length?s.skills.map(sk=>`<article><div><h3>${esc(sk.name)}</h3><p>${esc(sk.when)}</p></div><b>${sk.uses} uses</b></article>`).join(''):'<p class="empty">No skills yet.</p>');const mp=$('#mud-panel');const mudBody=$('#mud-body');const mudInput=$('#mud-input');if(s.mud&&s.mud.active){const myTurn=s.mud.roster&&s.mud.roster.some(a=>a.is_you&&a.is_turn);mudBody.innerHTML='<div class="mud-meta"><div><small>Scenario</small><b>'+esc(s.mud.scenario)+'</b></div><div><small>Room</small><b>'+esc(s.mud.room||'unknown')+'</b></div><div><small>Turn</small><b>'+esc(s.mud.turn||'—')+(s.mud.paused?' (paused)':'')+'</b></div></div><div id="mud-roster">'+(s.mud.roster||[]).map(a=>'<div class="mud-actor '+(a.is_you?'you ':'')+(a.is_turn?'turn ':'')+'"><b>'+esc(a.name)+'</b><small>'+esc(a.kind)+' · '+esc(a.room)+'</small><small>'+(a.inventory.length?'carrying: '+a.inventory.map(esc).join(', '):'empty-handed')+'</small></div>').join('')+'</div><div id="mud-log">'+(s.mud.log||[]).map(esc).join('\n')+'</div>';mudInput.placeholder=myTurn?'What do you do?':'Waiting for '+esc(s.mud.turn||'...');mudInput.disabled=s.mud.finished}else{mudBody.innerHTML='<p class="empty">No dungeon crawl active. Start one with <code>/mud</code> or <code>/mud scenario &lt;description&gt;</code>.</p>';mudInput.placeholder='No active game';mudInput.disabled=true}$('#orgs').innerHTML=s.nursery.organisms.map(n=>`<button data-org="${esc(n)}" class="${n===s.organism.name?'on':''}"><b>${esc(n)}</b><span>${n===s.organism.name?s.organism.state:'stored'}</span></button>`).join('');const p=s.extensions.pending;$('#mutation').innerHTML=p?`<div class="mutation"><small>PENDING MUTATION</small><p><b>${esc(p.kind)}</b></p><button data-mut="approve">Approve</button><button data-mut="reject">Reject</button></div>`:'';bindDynamic()}
...
function debounceTyping(){setTyping(true);const now=Date.now();const due=lastTyping===0||now-lastTyping>4000;clearTimeout(typingTimer);typingTimer=setTimeout(()=>{setTyping(false)},1200);if(due){lastTyping=now;sendTyping()}}
