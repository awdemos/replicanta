...
def test_anaphora_loop_counts_as_no_candidate(org, monkeypatch):
    ...
    are identical."""
    loop = (
        "The first was soft and static; this one should be sharp. "
...
def test_clean_candidate_strips_group_context_scaffolding():
    ...
    raw = (
        "The user just said: You are in a group chat with John, "
    ...
    assert _clean_candidate(raw) == "Rain makes the attic feel smaller, in a good way."
...
def test_clean_candidate_strips_rogue_thought_echo():
    ...
    from replicanta.arena import ROGUE_THOUGHT, _clean_candidate
